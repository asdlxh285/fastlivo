...
void RegistrationPCL<PointSource, PointTarget>::setInputSource(const PointCloudSourceConstPtr& cloud) {
  ...
  std::shared_ptr<const pcl::PointCloud<PointSource>> std_input(input_.get(), [input = input_](const pcl::PointCloud<PointSource>*) mutable {});
  source_tree_ = std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointSource>>>(std_input, KdTreeBuilderOMP(num_threads_));
  source_covs_.clear();
...
void RegistrationPCL<PointSource, PointTarget>::setInputTarget(const PointCloudTargetConstPtr& cloud) {
  ...
  std::shared_ptr<const pcl::PointCloud<PointTarget>> std_target(target_.get(), [target = target_](const pcl::PointCloud<PointTarget>*) mutable {});
  target_tree_ = std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointTarget>>>(std_target, KdTreeBuilderOMP(num_threads_));
  target_covs_.clear();
